...
$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
		url:path+"/sys/bill/getprovider",//请求的url
	...
	productDesc.on("focus",function(){
		validateTip(productDesc.next(),{"color":"#666666"},"* 请输入商品描述",false);
	}).on("blur",function(){
		if(productDesc.val() != null && productDesc.val() != ""){
			validateTip(productDesc.next(),{"color":"green"},imgYes,true);
		}else{
			validateTip(productDesc.next(),{"color":"red"},imgNo+" 商品描述不能为空，请重新输入",false);
		}
	...
	addBtn.on("click",function(){
		if(billCode.attr("validateStatus") != "true"){
			...
			productName.blur();
		}else if(productDesc.attr("validateStatus") != "true"){
			productDesc.blur();
